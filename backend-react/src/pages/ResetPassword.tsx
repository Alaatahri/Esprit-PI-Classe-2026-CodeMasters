...
export default function ResetPassword() {
  ...
  const handleSubmit = async () => {
    if (newPassword.length < 6) {
      setErrorMsg('6 caractères minimum.'); setStatus('error'); return;
    }
    if (newPassword !== confirmPassword) {
      setErrorMsg('Les mots de passe ne correspondent pas.'); setStatus('error'); return;
    }
    setStatus('loading');
    try {
  ...
  return (
    <div style={{ minHeight:'100vh', background:'#0f0f1a', display:'flex',
      ...
      <div style={{ background:'#1a1a2e', padding:'40px', borderRadius:'12px',
        ...
        <h2 style={{ color:'#F5A623', marginBottom:'8px' }}>Nouveau mot de passe</h2>
        {status === 'success' ? (
          <div style={{ textAlign:'center', paddingTop:'16px' }}>
            <p style={{ fontSize:'48px', margin:'0 0 16px' }}>✅</p>
            <p style={{ marginBottom:'20px' }}>Mot de passe modifié !</p>
          ...
          <>
            <label style={{ fontSize:'13px', color:'#ccc' }}>Nouveau mot de passe</label>
            ...
            <label style={{ fontSize:'13px', color:'#ccc' }}>Confirmer</label>
            <div style={{ position:'relative', margin:'8px 0 16px' }}>
