...
export class UserService {
  ...
  async findAllByRole(role: string, limit = 100): Promise<User[]> {
    return this.userModel
      .find({ role })
      .sort({ createdAt: -1 })
      .limit(limit)
  ...
  async login(email: string, mot_de_passe: string): Promise<SafeUser | null> {
    try {
      ...
      console.log(`[AUTH] DB Pwd: ${user.mot_de_passe} | Input Pwd: ${mot_de_passe}`);
      ...
      console.log(`[AUTH] Password matched. isEmailVerified: ${user.isEmailVerified}`);
  ...
  async findClientsByArtisan(artisanId: string): Promise<User[]> {
    if (!Types.ObjectId.isValid(artisanId)) {
      throw new BadRequestException('ID artisan invalide');
    }

    // On cherche tous les projets où cet utilisateur est soit expertId, soit artisan dans les candidatures
    const userIdObj = new Types.ObjectId(artisanId);
    const projects = await this.userModel.db.collection('projects').find({
      $or: [
        { expertId: userIdObj },
        { 'applications.artisanId': userIdObj }
      ]
    }).toArray();
